...
func (s *server) configeureRouter() {
	...
	s.router.HandleFunc("/user", s.handleUserUpdate()).Methods("PATCH")          // добавление полей для пользователя
	s.router.HandleFunc("/auth/login", s.handleSessionsCreate()).Methods("POST") // аутентификация пользователя через логи-пароль
...
// TODO:: необходимо закрыть данную реализацию от всех пользователей, чтобы её возможно было вызвать только после авторизации
